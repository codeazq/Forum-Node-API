...
User.attemptLogin = (userName, password, result) => {
  sql.query("SELECT * FROM users WHERE name = ?", [userName], (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(err, null);
    ...
    if (res.length) {
      const user = res[0]
      if (user.password === bcrypt.hashSync(password, saltRounds)) {
        // generate token and refresh token
        // send token and refresh token in result
      } else {
...
User.updatePassword = (id, password, result) => {
  sql.query(
    ...
    [bcrypt.hashSync(password, saltRounds), id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
